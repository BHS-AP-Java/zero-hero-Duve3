...
public class Person {
   ...
   * @param forced if true, will overwrite any existing item in the slot; if false, will only add if the slot is empty
   */
  public void giveItem(Item item, Boolean forced) {
    if (forced || this.inventory[this.selectedSlot] == null) {
      ...
      System.out.println(this.name + " tried to get a " + item.getName() + " but the slot is occupied.");
    }
  }
